import {
  SANDBOX_ENGINE_VERSION, emptyStore, validateStore, normalizeStore, appendResult, resolveReview,
...
const PRESENTATION_LABELS = {
  status: {
    ...
    reopened_due_to_normative_change: 'Revisión reabierta por cambio normativo'
  },
  reviewState: {
...
function loadStore() {
  ...
  try {
    ...
    // 122.30 · edición obligatoria del bridge (§5): normalizar antes de validar y
    // continuar con el store normalizado, no con el parsed 1.0 original. Sin este
    // cambio, subir STORE_SCHEMA a 1.1 en el módulo canónico rompería la carga del
    // panel de laboratorio aun cuando appendResult() y resolveReview() siguieran
    // siendo compatibles.
    const normalized = normalizeStore(parsed);
    const checked = validateStore(normalized);
