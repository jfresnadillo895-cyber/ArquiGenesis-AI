...
function setEnabled(value) {
  ...
  lastMessage = next ? 'Ensayo habilitado. Sólo se escribirá el sidecar sandbox.' : 'Ensayo apagado. No se preparan nuevas escrituras.';
...
function render() {
  ...
  grid.append(
    card('Resultado', result?.status || 'Sin registro', result ? `${result.verification_level} · estado trazable` : 'El Core continúa sin depender del sidecar.'),
    card('Evidencia', String(result?.evidence_links || 0), 'Vínculos conservados en registros, no en Memoria.'),
    card('Pendientes', String(result?.unresolved?.length || 0), result?.unresolved?.slice(0, 3).join(' · ') || 'Sin evaluación adjunta.'),
  ...
  for (const review of reviews) {
    ...
    row.append(el('strong', `${review.review_id} · ${review.state}`), el('p', `Cambios: ${review.changed_claims.join(', ')} · Afectadas: ${review.affected_claims.map(x => x.claim_id).join(', ') || 'ninguna'}`));
