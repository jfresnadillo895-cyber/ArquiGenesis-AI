...
(async()=>{
 ...
 /* PTBR-01: cambiar a pt-BR en caliente y repetir las mismas pruebas del wrapper de fetch --
    el header que llega a TODAS las APIs propias y el bloque de idioma inyectado en las
    llamadas a Anthropic deben reflejar pt-BR, sin tocar nada de lo ya probado arriba para EN. */
 I.setLocale('pt',{persist:false});
 await fetch('/api/organismos',{method:'GET',headers:{Authorization:'Bearer x'}});
 let h5=calls.at(-1).init.headers; chk('APIs propias reciben el header de locale en pt',h5.get?h5.get('x-comprender-locale')==='pt':h5['x-comprender-locale']==='pt');
 await fetch('/api/anthropic',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model:'x',messages:[{role:'user',content:'ola'}]})});
 let b6=JSON.parse(calls.at(-1).init.body); chk('anthropic sin system recibe el bloque de idioma en pt-BR',typeof b6.system==='string'&&b6.system.includes('IDIOMA DE INTERAÇÃO'));
 let base2=[{type:'text',text:'CANON',cache_control:{type:'ephemeral'}}];
 await fetch('/api/anthropic',{method:'POST',body:JSON.stringify({system:base2,messages:[]})});
 let b7=JSON.parse(calls.at(-1).init.body); chk('bloque de sistema cacheado preservado (pt)',Array.isArray(b7.system)&&b7.system[0].text==='CANON'&&b7.system[0].cache_control.type==='ephemeral');
 chk('instruccion de idioma pt-BR agregada al final',b7.system.at(-1).text.includes('IDIOMA DE INTERAÇÃO'));
 chk('getLocale() === pt tras el cambio',I.getLocale()==='pt');
