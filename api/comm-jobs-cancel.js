// api/comm-jobs-cancel.js — Corte A del Sistema de Acontecimientos, Comunicaciones y Notificaciones
// ---------------------------------------------------------------------------------------------
// QUE HACE
//   POST { job_id, reason?, version_conocida? } → intenta transicionar el trabajo a CANCELLED
//   via comm_transicionar_job(). Idempotente: si ya esta CANCELLED, devuelve ok sin duplicar
//   nada. Si el trabajo ya esta en un estado terminal incompatible (por ejemplo EXPIRED),
//   comm_transicion_valida() lo rechaza y la transicion rechazada queda igual auditada en
//   comm_job_transitions (accepted=false) -- no se le miente al llamador con un "ok" falso.
//
// AISLAMIENTO
//   organization_id se resuelve de la sesion, igual que en los demas archivos de este corte.
//   comm_transicionar_job ya valida que el job sea de esa cuenta antes de tocar nada.
//
// VARIABLES DE ENTORNO
//   SUPABASE_URL / SUPABASE_SECRET_KEY   (ya cargadas)

const registrar = (o) => console.log(JSON.stringify({ evento: 'comm_jobs_cancel', ...o }));
...
export default async function handler(req, res) {
  ...
  const reason = (cuerpo.reason != null) ? String(cuerpo.reason) : 'cancelado_por_usuario';
  ...
  try {
    ...
    registrar({ accion: r && r.ok ? 'cancelado' : 'rechazado', perfil: perfil.slice(0, 8), job_id: jobId, motivo: r ? r.motivo : null });
    ...
    registrar({ error: 'fallo_cancelar', perfil: perfil.slice(0, 8), job_id: jobId, detalle: String((e && e.message) || e) });
    return res.status(503).json({ error: { message: 'No se pudo cancelar. Volve a intentar.', codigo: 'servicio_no_disponible' } });
  }
}
