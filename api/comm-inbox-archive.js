// api/comm-inbox-archive.js — Corte B del Sistema de Acontecimientos, Comunicaciones y Notificaciones
// ---------------------------------------------------------------------------------------------
// QUE HACE
//   POST { entry_id } → archiva la entrada (comm_archivar_entrada). Idempotente. Archivar
//   NO borra la fila -- solo la saca de la vista activa por defecto (api/comm-inbox.js la
//   sigue devolviendo con ?incluir_archivadas=1). La evidencia se conserva siempre.
//
// VARIABLES DE ENTORNO
//   SUPABASE_URL / SUPABASE_SECRET_KEY   (ya cargadas)

const registrar = (o) => console.log(JSON.stringify({ evento: 'comm_inbox_archive', ...o }));
...
export default async function handler(req, res) {
  ...
  try {
    const r = await rpc('comm_archivar_entrada', { p_entry_id: entryId, p_organization_id: perfil }, SB_URL, SERVICE_KEY);
    registrar({ accion: r && r.ok ? 'archivada' : 'rechazado', perfil: perfil.slice(0, 8), entry_id: entryId });
    ...
    registrar({ error: 'fallo_archivar', perfil: perfil.slice(0, 8), entry_id: entryId, detalle: String((e && e.message) || e) });
    return res.status(503).json({ error: { message: 'No se pudo archivar. Volve a intentar.', codigo: 'servicio_no_disponible' } });
  }
}
