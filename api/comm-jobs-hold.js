// api/comm-jobs-hold.js — Corte A del Sistema de Acontecimientos, Comunicaciones y Notificaciones
// ---------------------------------------------------------------------------------------------
// QUE HACE
//   POST { job_id, reason?, version_conocida? } → transiciona el trabajo a HELD (retencion
//   autorizada) via comm_transicionar_job(). Solo funciona desde CREATED -- por diseño
//   (comm_transicion_valida): un trabajo ya en PROCESSING o en un estado terminal no se puede
//   retener retroactivamente, se rechaza y queda auditado igual.
//
// VARIABLES DE ENTORNO
//   SUPABASE_URL / SUPABASE_SECRET_KEY   (ya cargadas)

const registrar = (o) => console.log(JSON.stringify({ evento: 'comm_jobs_hold', ...o }));
...
export default async function handler(req, res) {
  ...
  const reason = (cuerpo.reason != null) ? String(cuerpo.reason) : 'retenido_por_usuario';
  ...
  try {
    ...
    registrar({ accion: r && r.ok ? 'retenido' : 'rechazado', perfil: perfil.slice(0, 8), job_id: jobId, motivo: r ? r.motivo : null });
    ...
    registrar({ error: 'fallo_retener', perfil: perfil.slice(0, 8), job_id: jobId, detalle: String((e && e.message) || e) });
    return res.status(503).json({ error: { message: 'No se pudo retener. Volve a intentar.', codigo: 'servicio_no_disponible' } });
  }
}
