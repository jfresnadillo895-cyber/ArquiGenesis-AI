...
async function accionEliminar(id, SB_URL, SERVICE_KEY, res) {
...
async function accionCancelarBaja(id, SB_URL, SERVICE_KEY, res) {
  try {
    const r = await fetch(SB_URL + '/rest/v1/perfiles?id=eq.' + id, {
      method: 'PATCH',
      headers: {
...
async function accionCancelarDowngrade(id, SB_URL, SERVICE_KEY, res) {
  try {
...
export default async function handler(req, res) {
  ...
  if (accion === 'eliminar') return accionEliminar(id, SB_URL, SERVICE_KEY, res);
  if (accion === 'cancelar_baja') return accionCancelarBaja(id, SB_URL, SERVICE_KEY, res);
  if (accion === 'cancelar_downgrade') return accionCancelarDowngrade(id, SB_URL, SERVICE_KEY, res);

  res.status(400).json({ error: { message: 'Acción inválida.', codigo: 'accion_invalida' } });
}
